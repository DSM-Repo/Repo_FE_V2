...
test('renders an empty selected class student list while no student data is loaded', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog').getByText('등록된 학생이 없습니다.')).toBeVisible()
  ...
  await classTrigger.click()
  await expect(page.getByRole('dialog').getByText('검색 결과가 없습니다.')).toBeVisible()
  await expect(page.getByRole('dialog').getByRole('link', { name: /레주메 보러가기/ })).toHaveCount(0)
})
